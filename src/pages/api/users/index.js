...
async function createUser(req, res) {
  try {
    const {
      ...
      nic: displayName,
      password,
      mm_slot: slotCommission,
      mm_live: liveCommission,
      bankinfo: bankName,
    ...
    if (!username || !displayName || !password || !accountHolder) {
    ...
    // Fetch the level of the selected user
    const levelQuery = 'SELECT level FROM user WHERE id = ?';
    const levelResult = await executeQuery(levelQuery, [selectedUserId]);
    ...
    const newLevel = levelResult[0].level + 1;
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({
        success: false,
        message: 'Username or display name already exists'
      });
    }
